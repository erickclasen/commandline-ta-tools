    ...
    with open(ticker_filename) as f:
        ...
        for row in reader:
            try:
                    ...
                    price_dict["BTC"] = float(row[1])
                    price_dict["DOGE"] = float(row[2])
                    price_dict["ETH"] = float(row[3])
                    price_dict["FET"] = float(row[4])
...
def asset_price_now(currency,price_list_of_dicts):
        ''' Pull the price of a currency right out of the list of dicts. '''
        return price_list_of_dicts[-1][currency+' Price']


def read_portfolio_size(filename='portfolio_size.json'):
...
def load_normed_risk_scaler():
    ''' Load the risk scaler dict from the /tmp file location. Return it.
        The json file is published periodically by the index-balancer code
        that does portfolio balancing. Runs this to create it. TBD may get rolled into
        this code to make it independant. If the file is not present, this scaling can
        be removed from this code and just forgo the return ratio risk -v- reward adjusment
        and have a straight all currencies trade the same amount philosophy.'''

    filename = "/tmp/risk_scaling_dict.json"
    try:
            with open(filename) as f_obj:
                    normed_risk_scaler_dict = json.load(f_obj)
    except IOError:
            raise Exception("Missing risk scaler dict created by index-balancer json file at ",filename)

    return normed_risk_scaler_dict
